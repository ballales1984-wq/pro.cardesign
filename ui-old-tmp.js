...
export class UI {
   ...
   _setupProceduralPanel() {
       ...
       if (rules.length === 0) {
        ...
        rules.forEach(ruleName => {
          ...
          ruleDiv.innerHTML = `
            ...
            <button id="rule-apply-${ruleName}" class="rule-apply-btn" data-rule="${ruleName}" title="Applica regola nella scena" style="background:var(--accent);color:#fff;border:none;border-radius:3px;padding:2px 7px;font-size:10px;cursor:pointer;">ÔûÂ Applica</button>
     ...
     addRuleBtn.addEventListener('click', function() {
       ...
       if (self.proceduralEngine.rules.has(ruleName)) {
         self._notify('Una regola con questo nome esiste gi├á', 'warn');
         return;
       }
    ...
    codeTextarea.value = rule.execute.toString();
      ...
      saveBtn.addEventListener('click', function() {
        try {
          ...
          // ÔÜá´©Å  SECURITY NOTE: `new Function` executes arbitrary code from the
          // textarea. This is intentional ÔÇö power-users need JS flexibility ÔÇö
  ...
  _showVoxelProperties(voxel) {
    ...
    // Dimension display ÔÇö use real scale from voxel data
    ...
    if (mat) {
      matInfo = '<div class="prop-row"><span class="prop-label">E (Young)</span><span class="prop-value">' + (mat.youngsModulus / 1e9).toFixed(1) + ' GPa</span></div>' +
        ...
        '<div class="prop-row"><span class="prop-label">Lambda termica</span><span class="prop-value">' + mat.thermalConductivity + ' W/(m┬ÀK)</span></div>' +
  ...
  async _runSimulation() {
    ...
    // ÔöÇÔöÇ Stress Analysis ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    try {
      ...
      html += '<hr style="border-color:var(--border);margin:8px 0;">' +
        '<div style="font-size:11px;color:var(--text-dim);margin-bottom:4px;">­ƒö¼ Stress Strutturale:</div>' +
    ...
    // ÔöÇÔöÇ Aerodynamics ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    try {
      ...
      html += '<hr style="border-color:var(--border);margin:8px 0;">' +
        '<div style="font-size:11px;color:var(--text-dim);margin-bottom:4px;">­ƒÆ¿ Aerodinamica (stima):</div>' +
  ...
  // ÔöÇÔöÇ Export Mesh ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  _openExportModal() {
    const self = this;
    ...
    overlay.innerHTML = `
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:28px;min-width:360px;box-shadow:0 24px 64px rgba(0,0,0,.6);">
        <h2 style="margin:0 0 18px;font-size:16px;">­ƒôª Esporta Mesh</h2>
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end;">
          ...
          <button id="ex-ok" style="padding:8px 18px;background:var(--accent);color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:600;">Ô¼ç Scarica</button>
        </div>
      </div>`;
    ...
    document.getElementById('ex-ok').onclick = async () => {
      ...
      btn.textContent = 'ÔÅ│ Generando...'; btn.disabled = true;
      try {
        ...
        btn.textContent = 'Ô¼ç Scarica'; btn.disabled = false;
      }
    };
  }

  // ÔöÇÔöÇ Import STL ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    _openImportModal() {
      var self = this;
      var input = document.createElement('input');
  ...
  _showImportResults(analysis, importInfo) {
    ...
    panel.innerHTML = `
      <div style="padding: 16px;">
        ...
        <div class="prop-row"><span class="prop-label">Ovalit├á</span><span class="prop-value">${analysis.ovalityMm} mm</span></div>
        ...
        <div class="prop-row"><span class="prop-label">Dim. Originali</span><span class="prop-value">${importInfo.originalSize.x.toFixed(1)} ├ù ${importInfo.originalSize.y.toFixed(1)} ├ù ${importInfo.originalSize.z.toFixed(1)}</span></div>
...
// Keyboard shortcuts
  _setupKeyboard() {
    ...
    document.addEventListener('keydown', function(e) {
      ...
      // Ctrl+Y or Ctrl+Shift+Z ÔåÆ Redo
  ...
  _addDemoVoxels() {
    ...
    console.log('%cÔùå VoxelCAD UI pronto. Strumenti: V=Seleziona A=Aggiungi R=Rimuovi F=Fill', 'color: #4caf50');
    this._notify('VoxelCAD pronto - Demo caricato', 'success');
  }

  // ÔöÇÔöÇ Component Library ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  _setupLibrary() {
    const self = this;
   ...
   _showComponentEditor(comp) {
     ...
     const panel   = document.getElementById('panel-component-selected');
      ...
      headerEl.innerHTML = '<div><div style="font-size:14px;font-weight:600;">' + comp.icon + ' ' + comp.name + '</div>' +
        ...
        '<button id="close-component-editor" class="btn-icon" aria-label="Chiudi editor componente" onclick="this.closest(\'.panel\').style.display=\'none\'">Ô£ò</button>';
